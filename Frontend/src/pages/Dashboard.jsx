...
const Dashboard = () => {
    ...
    const recentCompletedModules = Array.isArray(dashboardData?.recent_completed_modules)
        ? dashboardData.recent_completed_modules
        : [];
                ...
                <div className="lg:col-span-2 space-y-6">
                    <section>
                        ...
                        {continueCourse ? (
                            <div
                                onClick={() => {
                                    if (continueCourse.is_completed && continueCourse.has_certificate) {
                                        navigate(`/certificate/${continueCourse.course_id}`);
                                        return;
                                    }
                                    navigate(`/lesson/${continueCourse.course_id}`);
                                }}
                            ...
                            >
                                <Card className="flex flex-col sm:flex-row gap-6">
                                    <div className="w-full sm:w-1/3 bg-gray-200 rounded-lg h-32 sm:h-auto flex items-center justify-center text-gray-400">
                                        {continueCourse.is_completed ? 'Completed Course' : 'Active Course'}
                                    </div>
                                    <div className="flex-1 py-2">
                                        ...
                                        <p className="text-sm text-gray-600 mb-4 line-clamp-2">
                                            {continueCourse.is_completed
                                                ? continueCourse.has_certificate
                                                    ? 'Course completed. Your certificate is ready to view and download.'
                                                    : 'Course completed. Open the course to finish any remaining certificate step.'
                                                : 'Continue your active learning path from where you stopped.'}
                                        </p>
                                        ...
                                        {continueCourse.is_completed && continueCourse.has_certificate && (
                                            <div className="mt-4">
                                                <Button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                    ...
                    <section>
                        <h2 className="text-lg font-bold text-gray-800 mb-4">Completed Modules</h2>
                        {recentCompletedModules.length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                {recentCompletedModules.map((item) => (
                                    <Card key={`${item.course_id}-${item.module_id}`} className="p-4" hover={false}>
                                        <p className="text-xs font-semibold uppercase tracking-wide text-green-700 mb-1">Completed</p>
                                        <h3 className="font-bold text-sm text-gray-900">{item.module_title}</h3>
                                        <p className="text-xs text-gray-500 mt-1">{item.course_title}</p>
                                    </Card>
                                ))}
                            </div>
                        ) : (
                            <Card className="p-4">No completed modules yet.</Card>
                        )}
                    </section>
